...
// EtcdModel 操作etcd的接口，对etcd进行增删改查
type EtcdModel interface {
	...
	Transaction(ctx context.Context, op *TransactionOperator) (int64, error)
}
...
func (em *EtcdModelImpl) Transaction(ctx context.Context, op *TransactionOperator) (int64, error) {
	var conditions []clientv3.Cmp
	var ops []clientv3.Op
	updatedMessages := make(map[string][]byte)
	for _, m := range op.Adds {
		conditions = append(conditions, clientv3.Compare(clientv3.Version(m.Key), "=", 0))
		value, err := proto.Marshal(m.Message)
		if err != nil {
			return 0, err
		}
		updatedMessages[m.Key] = value
		ops = append(ops, clientv3.OpPut(m.Key, string(value)))
	}
	for _, m := range op.Updates {
		if m.Revision > 0 {
			conditions = append(conditions, clientv3.Compare(clientv3.ModRevision(m.Key), "=", m.Revision))
		}
		value, err := proto.Marshal(m.Message)
		if err != nil {
			return 0, err
	...
	for _, m := range op.Deletes {
		if m.Revision > 0 {
			conditions = append(conditions, clientv3.Compare(clientv3.ModRevision(m.Key), "=", m.Revision))
		}
		ops = append(ops, clientv3.OpDelete(m.Key, clientv3.WithPrevKV()))
	}
	if len(ops) == 0 {
		return 0, nil
	}
	c, cancel := context.WithTimeout(ctx, em.queryTimeout)
	defer cancel()
	resp, err := withLatency("transaction", func() (*clientv3.TxnResponse, error) {
		return em.client.Txn(c).If(conditions...).Then(ops...).Commit()
	})
	if err != nil {
		return 0, err
	}
	if !resp.Succeeded {
		return 0, &util.RevisionConflictError{}
	}
	// This is a fast path to dispatch etcd events to watchers.
	// It's necessary as the scheduler relies on the integrity of the watching indexes.
	var events []EtcdEvent
	for _, r := range resp.Responses {
		if rr := r.GetResponseDeleteRange(); rr != nil {
			for _, kv := range rr.PrevKvs {
				events = append(events, EtcdEvent{
	...
	for key, value := range updatedMessages {
		events = append(events, EtcdEvent{
	...
	em.watcher.dispatchEvents(events)
	return resp.Header.Revision, nil
}
